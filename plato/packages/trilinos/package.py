...
class Trilinos(CMakePackage, CudaPackage, ROCmPackage):
    ...
    variant(
        "cxxstd", default="14", description="C++ standard", values=["11", "14", "17", "20"], multi=False
    )
    ...
    variant("epetra", default=False, description="Compile with Epetra")
    variant("epetraext", default=False, description="Compile with EpetraExt")
    ...
    with when("~kokkos"):
        ...
        conflicts("+intrepid2")
        conflicts("+phalanx")
        conflicts('+kokkoskernels') # Added by Plato
    ...
    # Conflicts added by Plato
    conflicts('+percept', when='~boost') # Added by Plato
    ...
    conflicts('+krino', when='~boost') # Added by Plato
    # ###################### Dependencies ##########################
    ...
    depends_on("kokkos-kernels@4.7.00", when="+kokkoskernels")
    ...
    requires("+stk+stkbalance+intrepid2+zoltan2+boost", when="+krino") # Added by Plato
    ...
    patch("seacas_zoltan.patch", when="@15.0.0") # Added by Plato
    ...
    def cmake_args(self):
        ...
        options.extend(
            [
                ...
                define_tpl_enable('KokkosKernels', True), # added by Plato
                define_trilinos_enable('ShyLU_NodeTacho', 'tacho'), # added by Plato
                define_from_variant('Tacho_ENABLE_INT_INT', 'tacho'), # added by Plato
            ]
        )
        # Added by Plato
        if '+pamgen' in spec:
            # boost 1.79 not compiling with pamgen on gcc 7.5. Consider testing and removing when updating
            options.extend(['-DPamgen_ENABLE_Boost=OFF']) # added by Plato
        ...
        options.extend(
            [
                ...
                # Added by Plato
                define('Kokkos_ARCH_BDW', False), # added by Plato
                define('Kokkos_ARCH_HSW', False) # added by Plato
            ]
        )
